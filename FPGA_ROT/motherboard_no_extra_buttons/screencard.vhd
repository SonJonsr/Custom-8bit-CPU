...
ARCHITECTURE RTL OF screencard IS
  ...
  signal rst_n_dff_1 : std_logic := '1';
  signal rst_n_dff_2 : std_logic := '1';

BEGIN
  VGA_CLK <= pixel_clk;

  font_row <= (row mod 16) + 1  when (column > h_pixels - 4) else row mod 16;
  
  -- rst_n_syncer_screencard: process(pixel_clk) is
  -- begin 
  --   if rst_n /= rst_n_dff_1 then
  --     rst_n_dff_1 <= rst_n;
  --   end if;
  --   if rst_n_dff_1 /= rst_n_dff_2 then
  --     rst_n_dff_2 <= rst_n_dff_1;
  --   end if;
  -- end process;

  main_screencard : process(pixel_clk) is
