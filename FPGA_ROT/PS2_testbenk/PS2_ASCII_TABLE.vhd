...
ENTITY PS2_ASCII_TABLE IS
  PORT ( 
    ...
    -- In-/Outputs
    ascii_info : INOUT STD_LOGIC_VECTOR(7 downto 0) := x"00"
  );
...
ARCHITECTURE RTL OF PS2_ASCII_TABLE IS 
  TYPE ADD_REMOVE_MACHINE IS (IDLE, START_ARRAY, COUNT_ARRAY, AD_ARRAY, RM_ARRAY, START_CPU, READ_CPU, RM_ASCII_CPU, WRITE_CPU);
  ...
  SIGNAL new_ascii_rising_edge : STD_LOGIC := '0';
  SIGNAL en_rising_edge : STD_LOGIC := '0';
...
BEGIN
  ...
  conflict_signal_handler : process(new_ascii_rising_edge, en_rising_edge, state)
  begin 
    case state is
      when IDLE =>
        if (new_ascii_rising_edge = '1' and en_rising_edge = '1') then
          conflict_new_ascii <= '1';
        end if;

      when START_CPU|READ_CPU|RM_ASCII_CPU|WRITE_CPU =>
        conflict_en <= '0';
        if (new_ascii_rising_edge = '1') then
          conflict_new_ascii <= '1';
        end if;

      when START_ARRAY|COUNT_ARRAY|AD_ARRAY|RM_ARRAY =>
        conflict_new_ascii <= '0';
    ...
    if (rising_edge(clk)) then
      case state is
        when IDLE =>
          ...
          array_iterator <= 0;
          if (en_rising_edge = '1' or conflict_en = '1') then
            state <= START_CPU;
          elsif (new_ascii_rising_edge = '1' or conflict_new_ascii = '1') then
            state <= START_ARRAY;
          end if;
        ...
        when READ_CPU =>
          if (array_select_sig = '1') then
            ...
            if (ascii_table_size > 0) then
              -- Move the read ASCII to the back of the queue
              ascii_table(ascii_table_size - 1 downto 0) <= ascii_table(0) & ascii_table(ascii_table_size - 1 downto 1); 
              state <= RM_ASCII_CPU;
            else
              state <= IDLE;
            end if;
        ...
        when RM_ASCII_CPU =>
          ...
          if (remove_table(array_iterator) = ascii_temp) then
            remove_table(array_iterator) <= x"00"; 
            ascii_table(ascii_table_size - 1) <= x"00";
            ascii_table_size <= ascii_table_size - 1;
          end if;

          -- If one ASCII is removed or the array iterator gets to 15 set state to idle.
          if (array_iterator = 15 or remove_table(array_iterator) = ascii_temp) then
            state <= IDLE;
          end if;
  ...
  new_ascii_rising_edge_detector : rising_edge_detector
   port map(
      clk => clk,
      sig => new_ascii,
      rising_edge_detected => new_ascii_rising_edge
  );

  en_rising_edge_detector : rising_edge_detector
   port map(
      clk => clk,
      sig => en,
      rising_edge_detected => en_rising_edge
  );
