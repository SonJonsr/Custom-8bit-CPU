...
ARCHITECTURE RTL OF PS2_TO_ASCII IS 
  ...
  SIGNAL capitalized : STD_LOGIC_VECTOR;
...
BEGIN
  shft <= x"10" when (shft_r = '1' or shft_r = '1') else x"00";
  ctrl <= '1' when (ctrl_r = '1' or ctrl_r = '1') else '0';

  capitalized <= x"20" when (shft = x"10" or caps = '1') else x"00";
  ...
  begin
    if (rising_edge(clk)) then
      if (rst_n = '0') then
        ...
        if (new_code_rising_edge = '1') then
          ...
          if (layer = '0') then
            case code is
              ...
              -- NUMROW --
              when x"45" => ascii_sig <= x"30" when (shft = x"00") else 
                                     x"7d" when (alt_r = '0') else x"3D"; -- 0, =, }
              when x"16" => ascii_sig <= x"31" xor shft; -- 1, !
              when x"1E" => ascii_sig <= x"32" xor shft
                                      when (alt_r = '0') else x"40"; -- 2, ", @
              ...
              when x"3D" => ascii_sig <= x"37" when (shft = x"00") else 
                                     x"7B" when (alt_r = '1') else x"2F"; -- 7, /, {
              when x"3E" => ascii_sig <= x"38" xor shft 
                                      when (alt_r = '0') else x"5B"; -- 8, (, [
              when x"46" => ascii_sig <= x"39" xor shft
                                      when (alt_r = '0') else x"5D"; -- 9, ), ]

              -- PUNCTUATION --
              when x"41" => ascii_sig <= x"2C"; -- ,, ;
              when x"49" => ascii_sig <= x"2E"; -- ., :

              -- MATCH --
              when x"4A" => ascii_sig <= x"2E" when (shft = x"00") else x"5F"; -- -, _
              when x"55" => ascii_sig <= x"2B" when (shft = x"00") else x"3F"; -- +, ?
        ...
        elsif (new_code_rising_edge_dff = '1') then
