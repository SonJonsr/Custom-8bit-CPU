    ...
    if (rising_edge(pixel_clk)) then
      ...
      if (display_en = '1') then 
        ...
        case column mod 8 is 
          -- In the first pixel of the line finds the address
          when 0 => 
          ...
          -- In the second pixel start querying the RAM for the ASCII
          when 1 =>
            adr <= STD_LOGIC_VECTOR(base_display_adr + to_unsigned(adr_int, adr'LENGTH));

          -- In the fourth pixel save the ASCII queryed from the RAM, and start the query for
          -- attributes 
          when 3 =>
            ascii <= dat;
            adr <= STD_LOGIC_VECTOR(base_display_adr + to_unsigned(adr_int + 1, adr'LENGTH));

          -- In the sixth pixel save the attributes queryed from the RAM.
          when 5 =>
            attributes <= dat;

          -- In the eight pixel save the char_line for the next ASCII character into 
