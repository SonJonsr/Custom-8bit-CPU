...
-- SIGNALS FOR COMPONENTS --
  ...
  -- minne
      SIGNAL q_a		    : STD_LOGIC_VECTOR (9 DOWNTO 0) := (others => '0');
      SIGNAL address_a	: STD_LOGIC_VECTOR (15 DOWNTO 0) := (others => '0');
      SIGNAL clock_a		: STD_LOGIC := '0';
      SIGNAL data_a		  : STD_LOGIC_VECTOR (9 DOWNTO 0) := (others => '0');
...
BEGIN
  ...
  register_new_data <= (others => SW(0));
  register_old_data <= (others => SW(1));

  clock_a <= CLOCK_50 when mem_read_counter /= "00" else '0';
  ...
  sar_adc_register_test_inst: sar_adc_register_test
   port map(
      ...
      register_data_output => GPIO(15 downto 8),
      progress => progress,
      register_en => GPIO(16),
      register_rst => GPIO(17),
      register_data_input => GPIO(7 downto 0),
      q_b => q_b,
      address_b => address_b,
      clock_b => clock_b,
      data_b => data_b,
