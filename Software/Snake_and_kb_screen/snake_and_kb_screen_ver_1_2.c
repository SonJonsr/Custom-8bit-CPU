...
int16_t adr_info_display_minus_two = 0xEEFE;
...
int16_t enter_and_var = 0xFF80;
int8_t blink_black = 0xA0;
...
cursor_snake_start: // (*@\label{line:end_byte_conversion_setup}@*)
  ...
  keyboard_input_potential = kb_right;

  game_increment = five_hundred;
...
game_loop:
  ...
  if (keyboard_input != keyboard_input_illegal) {
    if (keyboard_input != direction) {
      ...
      if (new_input == def_one) {
        MEM[adr_keyboard_info] = def_one;
      }
      new_input = def_one;
      goto new_input_confirmed;
    }
  }
  new_input = clear;
...
delete_snake_head:
  ...
  if (snake_head_next_color == color_silver) {
    ...
    if (temp_eight_bit != snake_board_head) {
      goto game_over;
    }
  }
  ...
  score_draw:
    ...
    game_increment = game_increment - ten;
    if (game_increment < hundred) {
      game_increment = hundred;
    }
  }
...
get_new_input:
  ...
  if (keyboard_input != direction) {
    if (keyboard_input != keyboard_input_illegal) {
      keyboard_input_potential = keyboard_input;
    }
  }
dont_get_new_input:
...
timer_else: // END OF TIMER (*@\label{end:timer_millis_snake}@*)
  ...
  keyboard_input = keyboard_input_potential;
  goto game_loop;
...
keyboard_to_screen_start:
  i = adr_main_display;
clears_screen_complete_i:

  odd_check = i;
  odd_check = odd_check && one;
  if (odd_check != one) {
    MEM[i] = space;
    goto else_attr_complete;
  }
  MEM[i] = color_black;
else_attr_complete:
  i++;
  if (i != adr_info_display) {
    goto clears_screen_complete_i;
  }

  cursor = adr_main_display;

keyboard_to_screen:
  if (cursor > adr_info_display_minus_two) {
    cursor = adr_main_display;
  }
  if (cursor < adr_main_display) {
    cursor = adr_info_display_minus_two;
  }
  temp = cursor + one;
  MEM[temp] = blink_black;
keyboard_wait_for_key:
  temp_eight_bit = MEM[adr_keyboard_info];
  if (temp_eight_bit != def_zero) {
    temp = cursor + one;
    MEM[temp] = color_black;
    keyboard_input = MEM[adr_keyboard_ascii];
    MEM[adr_keyboard_info] = def_one;
    if (keyboard_input == escape) {
      goto snake_game;
    }
    if (keyboard_input == backspace) {
      cursor--;
      MEM[cursor] = color_black;
      cursor--;
      MEM[cursor] = space;
      goto keyboard_to_screen;
    }
    if (keyboard_input == enter) {
      cursor = cursor + screen_width;
      cursor = cursor && enter_and_var;
      goto keyboard_to_screen;
    }
    if (keyboard_input == tab) {
      cursor = cursor + four;
    ...
    goto keyboard_input_else;
  }
  goto keyboard_wait_for_key;

keyboard_input_else:
  MEM[cursor] = keyboard_input;
  cursor++;
  MEM[cursor] = color_black;
  cursor++;
  goto keyboard_to_screen;
